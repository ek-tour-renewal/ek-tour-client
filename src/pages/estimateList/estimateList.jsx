...
const EstimateList = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/estimate/all', {
      params: {
        page: page - 1,
      },
    })
      .then(response => {
        if (response.data.totalPage < parseInt(page) || 1 > parseInt(page)) throw new Error("해당 페이지를 찾을 수 없습니다");
        setRequestDataList(response.data.estimateList);
