...
const Estimate = (props) => {
  ...
  const validate = () => {
    ...
    if (
      regEmail.test(estimateForm.email) === false &&
      estimateForm.email.length > 0
    ) {
    ...
    if (
      /^[0-9]+$/.test(estimateForm.memberCount) === false &&
      estimateForm.memberCount.length > 0
    ) {
      ...
      <Stack sx={{ p: 4, pb: 0 }}>
        ...
        <Box sx={{ width: "200px", margin: "0 auto" }}>
          <Stack direction='row' justifyContent='flex-start'>
            <Typography variant='caption' color='gray' sx={{ width: "135px" }}>
              인원수
            </Typography>
          </Stack>
          <FormControl sx={{ mb: 1 }}>
