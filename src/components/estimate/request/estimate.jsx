...
const Estimate = () => {
  ...
  const [departPlaceDetailErrorMsg, setDepartPlaceDetailErrorMsg] =
    useState("");
  const [arrivalPlaceDetailErrorMSg, setArrivalPlaceDetailErrorMsg] =
    useState("");
  ...
  const validate = () => {
    ...
    if (
      regEmail.test(emailRef.current.value) === false ||
    ...
    if (
      !/^[0-9]+$/.test(memberCountRef.current.value) ||
      memberCountRef.current.value.length <= 0
    ) {
  ...
  const onSubmit = async (event) => {
    ...
    if (validate()) {
      ...
      const data = {
        ...
        departDate:
          departDateRef.current.value + "T" + departTimeRef.current.value,
        departPlace:
          departPlaceRef.current.value + departPlaceDetailRef.current.value,
        arrivalDate:
          arrivalDateRef.current.value + "T" + arrivalTimeRef.current.value,
        arrivalPlace:
          arrivalPlaceRef.current.value + arrivalPlaceDetailRef.current.value,
    ...
    <Paper elevation={10} sx={{ m: "2em 0", borderRadius: "15px" }}>
      ...
      <Stack sx={{ p: 4, pb: 0 }}>
        ...
        >
          <Typography
            variant='caption'
            sx={{ width: "100%", color: "gray", textAlign: "start" }}
          >
            인원수 *
          </Typography>
